...
RUN	curl -H "Cache-Control: no-cache" https://raw.githubusercontent.com/Wizkiller96/Wizdecker/master/wizbot_installer_2_28_3.sh -o ./wizbot_installer.sh && chmod 755 wizbot_installer.sh && ./wizbot_installer.sh
RUN	curl -O -H "Cache-Control: no-cache" https://raw.githubusercontent.com/Wizkiller96/Wizdecker/master/wizbot_autorestart.sh && chmod 755 wizbot_autorestart.sh

VOLUME ["/root/wizbot"]

CMD ["sh","/opt/wizbot_autorestart.sh"]